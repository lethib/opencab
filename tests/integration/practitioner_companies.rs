...
use opencab::{
  ...
  config::Config,
  models::_entities::{company_interventions, practitioner_companies, users},
};
...
mod delete_a_company {
  ...
  mod when_the_company_belongs_to_another_user {
    ...
    async fn then_it_returns_403() {
      ...
      // Then
      assert_eq!(response.status(), StatusCode::FORBIDDEN);
      let still_there = practitioner_companies::Entity::find_by_id(company.id)
        .one(&app.db)
        .await
        .unwrap();
      assert!(still_there.is_some(), "company should not have been deleted");
    }
  }
  ...
  mod when_the_user_owns_the_company {
    ...
    #[tokio::test]
    async fn then_it_deletes_the_company_and_returns_204() {
      // Given
      let app = setup_http().await;
      let user = UserFactory::new().create(&app.db).await;
      let company = CompanyFactory::new().create_for_user(&app.db, user.id).await;
      ...
      // Then
      assert_eq!(response.status(), StatusCode::NO_CONTENT);
      let deleted = practitioner_companies::Entity::find_by_id(company.id)
        .one(&app.db)
        .await
        .unwrap();
      assert!(deleted.is_none(), "company should have been deleted");
      let intervention_deleted = company_interventions::Entity::find_by_id(intervention.id)
        .one(&app.db)
        .await
        .unwrap();
      assert!(
